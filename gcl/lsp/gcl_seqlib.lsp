...
;; 		      (otherwise ,n)))))
	   ...
	   (defmacro i-aref (n a i)
	     `(case ,n
		    ,@(let ((k -1) (d t)) 
			(delete nil 
				(mapcar (lambda (x) 
					  (cond ((search "NON-NEGATIVE-" (string x)) (setq d nil))
						((prog1 `(,(if d (incf k) `(,(incf k),(incf k)))
							  (let ((,a ,a))
							    (declare ((array ,x) ,a))
							    (aref ,a ,i)))
						   (setq d t))))) +array-types+)))))
  ...
  (let* ((aet (unless l (array-element-type s)))
	...
	((or (>= i end) (when l (endp p)))
	 (let ((r (cond (l (nconc r h))
	   ...
	   (if from-end (nreverse r) r)))
      (declare (seqind ri))
      (let* ((el1 (if l (car p) (i-aref ac s i)))
	...
	(when
	    (do ((j (1+ i) (1+ j))
		 ...
		 (test-comp (bump-test test-comp el1)))
		((or (>= j end) (when l (endp q))))
	      (let* ((el2 (if l (car q) (i-aref ac s j)))
...
(defmacro garef (a i l) `(let ((ff (aref ,a ,i))) (if ,l (car ff) ff)))
(defmacro raref (a i j l) `(if ,l (rotatef (car (aref ,a ,i)) (car (aref ,a ,j))) (rotatef (aref ,a ,i) (aref ,a ,j))))
...
(defun sort (seq pred &key (key 'identity))
  ...
  (let* ((ll (length seq))
	...
	(let* ((ls (aref ii (decf s))) (fi (aref ii (decf s))))
	  ...
	  (do nil ((>= fi (1- ls)))
	    (let ((n (+ fi (truncate (- ls fi) 2)))) (raref a fi n list))
	    (let* ((spi fi)
		   (sp (funcall key (garef a spi list))))
	      (declare (seqind spi))
	      (do ((lf fi) (rt ls)) ((>= lf rt))
		(declare (seqind lf rt))
		(do nil ((or (>= (incf lf) rt)
			     (not (funcall pred (funcall key (garef a lf list)) sp)))))
		(do nil ((or (>= lf (decf rt))
			     (funcall pred (funcall key (garef a rt list)) sp))))
		(when (< lf rt)
		  (raref a lf rt list))
		(setq spi (1- lf)))
	      (raref a fi spi list)
		...
		(when b1
		  (setf (aref ii s) ns (aref ii (incf s)) ns1 s (1+ s)))
		(setq ls nls fi nfi))))))
      seq)))
