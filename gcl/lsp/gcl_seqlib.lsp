...
(defun every (pred &rest seqs &aux (end (seqend seqs)))
  (declare (optimize (safety 2))(:dynamic-extent seqs))
  (labels ((loop (i ns vals) (or (>= i end)
				 (when (apply pred (seqvals vals ns))
				   (loop (1+ i) ns vals)))))
	  (loop 0 (dyncpl seqs) (dyncpl seqs))))
...
(defun some (pred &rest seqs &aux (end (seqend seqs)))
  (declare (optimize (safety 2))(:dynamic-extent seqs))
  (labels ((loop (i ns vals) (unless (>= i end)
			       (or (apply pred (seqvals vals ns))
				   (loop (1+ i) ns vals)))))
	  (loop 0 (dyncpl seqs) (dyncpl seqs))))
