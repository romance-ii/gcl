...
static void print_region (struct region_t *region)
{
  printf ("0x%8lx - 0x%8lx, length: 0x%8lx, protection: %c%c%c, max_protection: %c%c%c\n",
	  (unsigned long) region->address,
          (unsigned long) (region->address + region->size),
...
void unexec (char *outfile, char *infile, int dummy1, int dummy2, int dummy3)
{
...
build_symbol_table(void) {
    fprintf(stderr,"******** build_symbol_table(void) not implemented\n");
}
*/

void
unlink_loaded_files(void) {
    fprintf(stderr,"******** unlink_loaded_files(void) not implemented\n");
}
