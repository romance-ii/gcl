...
/* 	reloc = xballoc(b,storage_needed); */
...
/*       sgot->orelocation = xballoc(b, info->gotsize*sizeof(void*)); */
...
static void
mipself_special(bfd *b,asymbol **syms,int nsyms)
{
  /* MIPSELF case */
  if (bfd_get_flavour(b) == bfd_target_elf_flavour &&
      bfd_get_arch(b) == bfd_arch_mips )
    {
