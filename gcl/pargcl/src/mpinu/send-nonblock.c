...
#ifdef LOG_STRING
        { char filename[100];
          sprintf(filename, "mpinu%d.log", MPINU_myrank);
          init_log(filename);
	}
#endif
#ifdef NEW
...
# endif
    ...
    assert(DBG_COUNT==0);
# ifdef LOG_STRING
    if (send_len == sizeof(struct msg_hdr))
    log_string(4, "MPINU_send_nonblock(rank %d, sending): msg_hdr:"
		  " size: %d, tag: %d, rank: %d\n",
		MPINU_myrank,
		ntohl( ((struct msg_hdr*)buf)->size ),
		ntohl( ((struct msg_hdr*)buf)->tag ),
		ntohl( ((struct msg_hdr*)buf)->rank ) );
# endif
#endif
    DBG_COUNT += len;
    assert( pthread_mutex_unlock( &send_mutex ) == 0 );
...
#if defined(LOG_STRING) && !defined(NEW)
        if (send_len == sizeof(struct msg_hdr))
            log_string(4, "MPINU_send_nonblock(rank %d, dest %d, sent):"
			  " msg_hdr: size: %d, tag: %d, rank: %d\n",
		       MPINU_myrank, MPINU_rank_from_socket(send_sd),
		       ntohl( ((struct msg_hdr*)send_tmp_buf)->size ),
		       ntohl( ((struct msg_hdr*)send_tmp_buf)->tag ),
		       ntohl( ((struct msg_hdr*)send_tmp_buf)->rank ) );
        else
            log_string(4, "MPINU_send_nonblock(rank %d, dest %d, sent):"
			  " msg body: size: %d\n",
