  ...
  (let ((fin (allocate-funcallable-instance-1)))
    (set-funcallable-instance-function
     ...
     (fin-lambda-fn (&rest args)
       (declare (ignore args))
       (error "The function of the funcallable-instance ~S has not been set" fin)))
...
(defun bootstrap-built-in-classes ()
  ...
  (let ((class-eq-wrapper (class-wrapper (find-class 'class-eq-specializer))))
    ...
    ; FIXME use regular destructuring-bind
      (destructuring-bind (name supers subs cpl &optional prototype) e
	(let* ((class (find-class name))
	       #+cmu17
