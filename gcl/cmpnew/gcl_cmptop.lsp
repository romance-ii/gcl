...
(defun decl-safety (d &optional s)
  (cond ((consp (car d)) (max (decl-safety (car d) s) (decl-safety (cdr d) s)))
	...
	((and s (= s 2) (eq (car d) 'safety)) (cadr d))
	(0)))
...
(defun c1ub (args)
  (let* ((key (pop args))
	 (info (make-info :type #topaque :flags (iflags side-effects)))
...
(defun do-fun (name src e vis b)
  (let* ((*vars*   (when b (cons b *vars*)))
	 ...
	 (tag (tmpsym))
	 (*src-inline-recursion* (when vis (list (list (list name) tag (ttl-ll (cadr src))))))
	 (*c1exit* (list name))
...
(defun global-type-bump (tp)
  (let* ((mv (cmpt tp))
	 ...
	 (tppn (car (member tpp `(,@+c-global-arg-types+ ,#tt ,#t*) :test 'type<=))))
...
(defun t3defun-local-entry (fname cfun lambda-expr sp inline-info
  ...
  (let* ((cm *reservation-cmacro*)
	 ...
	 (*recur-stack* (if tri (cons tri *recur-stack*) *recur-stack*))
