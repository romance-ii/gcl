...
(defun co1typep (f args &aux tem) f
  ...
  (let* ((x (car args)) new
	 (info (make-info))
	 (nargs (c1args args info))
	 (etype (car (atomic-tp (info-type (cadadr nargs)))))
	 ;FIXME
	 (etype (when etype (cgss etype)));(unless (and (consp etype) (member-if (lambda (x) (or (eq x +opaque+) (when (symbolp x) (get x 'tmp)))) etype)) etype))
	 (typen (when etype (min-ftp etype)))
	 (typex (when etype (max-vtp etype)))
	 (rt (info-type (cadar nargs)))
	 (type typex))
    (setq new
	  (cond
	   ((not (and (single-type-p typen) (single-type-p typex))) nil)
	   ((type>= typen rt)
	   ...
	   ((and (consp etype) (eq (car etype) 'or) 
		 (not (member 'cons etype :key (lambda (x) (when (consp x) (car x)))))
		 (not (member-if (lambda (x) (when (symbolp x) (eq (symbol-package x) (find-package "PCL")))) etype)));FIXME
	    `(typecase ,x ,@(mapcar (lambda (y) `(,y t)) (cdr etype))))
	   ((and (consp etype) (member (car etype) '(and or not)))
	    `(,(car etype) ,@(mapcar (lambda (y) `(typep ,x ',y)) (cdr etype))))
	   ((and (consp etype) (eq (car etype) 'satisfies)) `(,(cadr etype) ,x))
	   ((and (consp etype) (eq (car etype) 'member)) `(when (member ,x ',(cdr etype)) t))
	   ((null type) nil)
	   ((and (eq typex typen) (setq f (assoc type +cmp-type-alist+))
		 (not (get (cdr f) 'si::struct-predicate)))
	    (list (cdr f) x))
	   ((type>= #tfixnum  type)
	    (let* ((tem (cmp-norm-tp type))(i (pop tem))(s (pop tem))(h (car tem)))
	      (setq tem (cmp-norm-tp type))
	      (when (and (si::fixnump s) (si::fixnump h))
		`(when (si::fixnump ,x) ,(if (= s h) `(= ,x ,s) `(<= ,s ,x ,h))))))
	   ((and (symbolp etype)
	   ...
	   ((let ((y (si::expand-deftype etype))) (when y `(typep ,x ',y))))
...
;; 	 (nargs (c1args args info))
;; 	 (etype (car (atomic-tp (info-type (cadadr nargs)))))
;; ;	 (etype (when (literalp (cadr args)) (lit-eval (cadr args))))
;; 	 (typen (when etype (min-ftp etype)))
;; 	 (typex (when etype (max-vtp etype)))
;; 	 (rt (info-type (cadar nargs)));(rt (result-type (car args)))
...
;; 	   ((and typex (not (type-and typex rt)))
;; 	    (ignorable-pivot (car args) (ignorable-pivot (cadr args) nil)))
;; ;	   ((when (symbolp etype) (setq f (get etype 'si::type-predicate)))
;; ;	    (return-from co1typep (c1expr (list f x))));FIXME deftype expansion -- handles satisfies for now
;; 	   ((not (eq typex typen)) nil)
;; 	   ((null type) nil)
...
;; 		 `(when (si::fixnump ,x) (<= ,(second tem) ,x ,(third tem)))
;; 		 ;; `(let ((.tem ,x))
