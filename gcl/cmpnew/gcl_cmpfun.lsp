    ...
    (let* ((eltp (position :element-type args))
      ...
      (let ((szf (let ((st (info-type (cadar nargs))))
		   (cond ((and (constantp (car args)) 
				 ...
				 (and (listp v) `(,(make-list (length v) :initial-element '*))))))
;			 ((type>= 'list st) `(,(make-list (cons-type-length st) :initial-element '*)))
...
;	(format t "~a ~a ~a ~a~%" type rt ta (eq ta rt))
	(cond ((eq ta rt)
	       (setq new t)
	       (return-from co1typep (c1t)))
	      ((and type (not ta))
	       (setq new nil)
	       (return-from co1typep (c1nil)))))
    (setq new
	  (cond
	   ((null type) nil)
