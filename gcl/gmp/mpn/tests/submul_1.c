     ...
     char **argv;
{
  ...
  for (test = 0; ; test++)
      ...
      if (test % (SIZE > 10000 ? 1 : 10000 / SIZE) == 0)
	{
      ...
      t0 = cputime();
      for (i = 0; i < TIMES; i++)
	refmpn_submul_1 (dx+1, s1, size, xlimb);
      t = cputime() - t0;
      cyc = ((double) t * CLOCK) / (OPS * 1000.0);
      printf ("refmpn_submul_1: %5ldms (%.2f cycles/limb) [%.2f Gb/s]\n",
	      t, cyc,
      ...
      t0 = cputime();
      for (i = 0; i < TIMES; i++)
	mpn_submul_1 (dx+1, s1, size, xlimb);
      t = cputime() - t0;
      cyc = ((double) t * CLOCK) / (OPS * 1000.0);
      printf ("mpn_submul_1:    %5ldms (%.2f cycles/limb) [%.2f Gb/s]\n",
	      t, cyc,
	      CLOCK/cyc*BITS_PER_MP_LIMB*BITS_PER_MP_LIMB/1e9);
#endif
